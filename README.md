...
| 频谱知识库 | MinerU 3.3.0 解析 4656 个 PDF，1.4M vectors ChromaDB + 9K 实体知识图谱。支持向量/关键词/图谱混合检索、LLM Query 重写、Rerank、流式问答和 PDF 来源查看 | `frontend/src/pages/KnowledgePage.jsx`, `/api/rag/*` |
...
|------|------|
| 文档数 | 4,656（去重后） |
| ChromaDB 向量 | 1,399,856 |
| 知识图谱实体 | 9,239 (Document/FrequencyBand/Standard/Footnote) |
| 知识图谱关系 | 16,259 |
| 解析引擎 | MinerU 3.3.0 pipeline |
| Embedding 模型 | BAAI/bge-m3 (1024-dim) |
| 检索模式 | 向量 + 关键词 + 图谱并行，LLM Query 重写 + Rerank |
| 块类型 | text / table / equation / chart / list / page_footnote / code |

### RAG 检索流程

```
用户 Query → LLM Query 重写 → 向量检索 + 关键词检索 + 图谱检索 (并行)
  → Rerank (频段/区域/标准号匹配加权) → Context Pack → LLM 答案生成
```
...
| 模块 | 使用状态 | 代码情况 | 注意事项 |
| --- | --- | --- | --- |
| 前端工作台 | 可本地运行，默认入口为 Console，Knowledge、Memory、System 与技能页面已接入 | React + Vite，页面状态保留与后台刷新已优化 | 生产构建使用 `npm --prefix frontend run build` |
| 后端 API | FastAPI 路由已覆盖 Chat、RAG、Memory、Spectrum Construction、Spectrum Decision | `backend/app.py` 统一挂载路由，`backend/api/system.py` 支持更多文本预览格式 | 需要 Python 依赖和本地/服务器数据目录完整 |
| Agent / LLM | 支持 OpenAI-compatible、Anthropic-compatible、DeepSeek、Qwen 等 provider | 系统提示词会扫描 `backend/skills/`，区分已实现与规划中 skill，并读取最新进化摘要 | 需要配置 `.env` 中的 LLM provider、base URL、API key 与模型 |
| RAG 知识库 | 已完成大规模 MinerU 3.3.0 解析、ChromaDB 入库和知识图谱重建 | 1,399,856 vectors、9,239 entities、16,259 relations；支持混合检索、Query 重写和 Rerank | 大型原始 PDF、向量库和图谱通常不随源码仓库发布 |
| 频率规划 | 可通过专用 RAG profile 输出带引用规划建议 | 已接入 `/api/rag/frequency_plan/stream` 和前端页面 | 答案质量依赖本地知识库数据和 LLM 可用性 |
| 频谱构建 | 物理预览可用，GenSpectra/UAV REM 走外部产物适配 | Gudmundson 预览、GenSpectra sidecar/子进程、UAV REM 只读适配 | 缺少 checkpoint 或外部实验目录时会降级 |
| 频谱决策 | 参数化分配与智能体流式解析可用 | CQI-Shannon + SLSQP 比例公平优化，带贪心吞吐基线 | LLM 解析自然语言需求时依赖 provider 可用 |
| 记忆与进化 | 会话、反馈、skill run 和进化报告以 best-effort 记录 | SQLite memory store 与前端 Memory 页面已接入 | 记忆失败不会阻塞主任务 |
| 链路脚本 | 本地、后端、前端、LLM bridge 和 links 脚本已提供 | `scripts/local/start_*.sh`、`scripts/local/start_links.sh` | SSH 隧道、代理和服务器路径依赖本机环境 |
